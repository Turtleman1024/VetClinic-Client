...
export function* fetchPatients() {
  const endpoint =  baseAddress + '/patients/active';
...
export function* fetchPatientById(action) {
  const endpoint = baseAddress + `/patient/id/${action.patientId}`;
  const response = yield call(fetch, endpoint);
  const data = yield response.json();
...
export function* updatePatient(action) {
  const endpoint = baseAddress + `/patient/${action.patientId}`;
...
export function* createNewPatient(action) {
  const endpoint = baseAddress + '/patient';
  const response = yield fetch(endpoint, {
    method: 'POST',
